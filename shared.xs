...
#define SHAREDSV_FROM_OBJ(sv) ((SvROK(sv)) ? INT2PTR(volatile SV *, SvIV(SvRV(sv))) : NULL)
 ...
 */
STATIC user_lock *
S_get_userlock(pTHX_ volatile SV* ssv, bool create)
{
    MAGIC *mg;
    user_lock *ul = NULL;
    ...
    for (mg = SvMAGIC((SV *)ssv); mg; mg = mg->mg_moremagic) {
    ...
    if (mg) {
        ...
        mg = sv_magicext((SV *)ssv, NULL, PERL_MAGIC_ext, &sharedsv_userlock_vtbl,
                            (char *)ul, 0);
 ...
 */
static void
Perl_sharedsv_associate(pTHX_ SV *sv, volatile SV *ssv)
{
    MAGIC *mg = 0;
    ...
    case SVt_PVHV:
        if (!(mg = mg_find(sv, PERL_MAGIC_tied))
            || mg->mg_virtual != &sharedsv_array_vtbl
            || (volatile SV*) mg->mg_ptr != ssv)
        {
            SV *obj = newSV(0);
            sv_setref_iv(obj, "threads::shared::tie", PTR2IV((SV *)ssv));
            if (mg) {
...
S_sharedsv_new_shared(pTHX_ SV *sv)
{
    ...
    SHARED_CONTEXT;
    ssv = newSV(0);
    SvREFCNT((SV *)ssv) = 0; /* Will be upped to 1 by Perl_sharedsv_associate */
    sv_upgrade((SV *)ssv, SvTYPE(sv));
    CALLER_CONTEXT;
    Perl_sharedsv_associate(aTHX_ sv, (SV *)ssv);
    return (ssv);
}
 ...
 */
STATIC SV *
S_sharedsv_new_private(pTHX_ volatile SV *ssv)
{
    SV *sv;
...
STATIC void
S_sharedsv_dec(pTHX_ volatile SV* ssv)
{
    if (! ssv)
        return;
    ENTER_LOCK;
    if (SvREFCNT((SV *)ssv) > 1) {
 ...
 */
STATIC void
S_get_RV(pTHX_ SV *sv, volatile SV *sobj) {
    SV *obj;
    if (! (SvROK(sv) &&
           ((obj = SvRV(sv))) &&
           (Perl_sharedsv_find(aTHX_ obj) == sobj) &&
           (SvTYPE(obj) == SvTYPE((SV *)sobj))))
    {
        /* Can't reuse obj */
        if (SvROK(sv)) {
    ...
    if (SvOBJECT((SV *)sobj)) {
        ...
        STRLEN len;
        char* stash_ptr = SvPV((SV*) SvSTASH((SV *)sobj), len);
...
#define get_RV(sv, sobj)                     \
        ...
        if (SvROK((SV *)sobj)) {                 \
            SvROK_on(SvRV(sv));                  \
            S_get_RV(aTHX_ SvRV(sv), SvRV((SV *)sobj)); \
        }
...
{
    volatile SV *ssv = (volatile SV *) mg->mg_ptr;
    assert(ssv);

    ENTER_LOCK;
    if (SvROK((SV *)ssv)) {
      get_RV(sv, SvRV((SV *)ssv));
    } else {
      sv_setsv_nomg(sv,(SV *)ssv);
    }
    LEAVE_LOCK;
    return (0);
}
 ...
 */
static void
sharedsv_scalar_store(pTHX_ SV *sv, volatile SV *ssv)
{
    dTHXc;
    bool allowed = TRUE;

    assert(PL_sharedsv_lock.owner == aTHX);
    if (!PL_dirty && SvROK((SV *)ssv) && SvREFCNT(SvRV((SV *)ssv)) == 1) {
        SV *sv = sv_newmortal();
    ...
    if (SvROK(sv)) {
        SV *obj = SvRV(sv);
        volatile SV *sobj = Perl_sharedsv_find(aTHX_ obj);
        if (sobj) {
            SHARED_CONTEXT;
            (void)SvUPGRADE((SV *)ssv, SVt_RV);
            sv_setsv_nomg((SV *)ssv, &PL_sv_undef);

            SvRV_set((SV *)ssv, SvREFCNT_inc((SV *)sobj));
            SvROK_on((SV *)ssv);
            if (SvOBJECT((SV *)sobj)) {
                /* Remove any old blessing */
              SvREFCNT_dec(SvSTASH((SV *)sobj));
                SvOBJECT_off((SV *)sobj);
            }
            if (SvOBJECT(obj)) {
              SV* fake_stash = newSVpv(HvNAME_get(SvSTASH(obj)),0);
              SvOBJECT_on((SV *)sobj);
              SvSTASH_set((SV *)sobj, (HV*)fake_stash);
            }
            CALLER_CONTEXT;
        } else {
            allowed = FALSE;
        ...
        SvTEMP_off(sv);
        SHARED_CONTEXT;
        sv_setsv_nomg((SV *)ssv, sv);
        if (SvOBJECT((SV *)ssv)) {
            /* Remove any old blessing */
          SvREFCNT_dec(SvSTASH((SV *)ssv));
          SvOBJECT_off((SV *)ssv);
        }
        if (SvOBJECT(sv)) {
          SV* fake_stash = newSVpv(HvNAME_get(SvSTASH(sv)),0);
          SvOBJECT_on((SV *)ssv);
          SvSTASH_set((SV *)ssv, (HV*)fake_stash);
        }
        CALLER_CONTEXT;
    }
    if (!allowed) {
...
{
    dTHXc;
    volatile SV *saggregate = SHAREDSV_FROM_OBJ(mg->mg_obj);
    volatile SV** svp = NULL;

    ENTER_LOCK;
    if (saggregate) {  /* During global destruction, underlying
                          aggregate may no longer exist */
      if (SvTYPE((SV *)saggregate) == SVt_PVAV) {
            assert ( mg->mg_ptr == 0 );
            SHARED_CONTEXT;
            svp = (volatile SV**)av_fetch((AV*) saggregate, mg->mg_len, 0);
        } else {
            char *key = mg->mg_ptr;
            I32 len = mg->mg_len;
            assert ( mg->mg_ptr != 0 );
            ...
            SHARED_CONTEXT;
            svp = (volatile SV**)hv_fetch((HV*) saggregate, key, len, 0);
        }
        CALLER_CONTEXT;
    }
    if (svp) {
...
sharedsv_elem_mg_STORE(pTHX_ SV *sv, MAGIC *mg)
{
    ...
    if (SvTYPE((SV *)saggregate) == SVt_PVAV) {
        assert ( mg->mg_ptr == 0 );
        SHARED_CONTEXT;
        svp = (volatile SV**)av_fetch((AV*) saggregate, mg->mg_len, 1);
    } else {
        char *key = mg->mg_ptr;
        I32 len = mg->mg_len;
        assert ( mg->mg_ptr != 0 );
        ...
        SHARED_CONTEXT;
        svp = (volatile SV**)hv_fetch((HV*) saggregate, key, len, 1);
    }
    CALLER_CONTEXT;
    Perl_sharedsv_associate(aTHX_ sv, *svp);
    sharedsv_scalar_store(aTHX_ sv, *svp);
    SvFLAGS((SV *)*svp) |= dualvar_flags;
    LEAVE_LOCK;
    return (0);
}
...
{
    PERL_UNUSED_ARG(param);
    SvREFCNT_inc_void((SV *)SHAREDSV_FROM_OBJ(mg->mg_obj));
...
{
    dTHXc;
    volatile SV *ssv = (SV *) mg->mg_ptr;
    U32 val;
    PERL_UNUSED_ARG(sv);
    SHARED_EDIT;
    if (SvTYPE((SV *)ssv) == SVt_PVAV) {
        val = av_len((AV*) ssv);
    } else {
...
sharedsv_array_mg_CLEAR(pTHX_ SV *sv, MAGIC *mg)
{
    dTHXc;
    volatile SV *ssv = (SV *) mg->mg_ptr;
    const bool isav = SvTYPE((SV *)ssv) == SVt_PVAV;
    PERL_UNUSED_ARG(sv);
    SHARED_EDIT;
    if (!PL_dirty) {
...
{
    PERL_UNUSED_ARG(sv);
    S_sharedsv_dec(aTHX_ (volatile SV*)mg->mg_ptr);
    return (0);
}
 ...
 */
static void
Perl_sharedsv_lock(pTHX_ volatile SV *ssv)
{
    user_lock *ul;
    if (! ssv)
        return;
...
PUSH(SV *obj, ...)
    CODE:
        dTHXc;
        volatile SV *sobj = SHAREDSV_FROM_OBJ(obj);
        int ii;
        for (ii = 1; ii < items; ii++) {
            ...
            SHARED_CONTEXT;
            av_push((AV*) sobj, (SV *)stmp);
            SvREFCNT_inc_void((SV *)stmp);
            SHARED_RELEASE;
            SvREFCNT_dec(tmp);
        }
...
UNSHIFT(SV *obj, ...)
    CODE:
        ...
        for (ii = 1; ii < items; ii++) {
            SV *tmp = newSVsv(ST(ii));
            U32 dualvar_flags = DUALVAR_FLAGS(tmp);
            volatile SV *stmp = S_sharedsv_new_shared(aTHX_ tmp);
            sharedsv_scalar_store(aTHX_ tmp, stmp);
            SHARED_CONTEXT;
            SvFLAGS(stmp) |= dualvar_flags;
            av_store((AV*) sobj, ii - 1,(SV *)stmp);
            SvREFCNT_inc_void((SV *)stmp);
            CALLER_CONTEXT;
            SvREFCNT_dec(tmp);
        }
...
POP(SV *obj)
    CODE:
        ...
        ENTER_LOCK;
        SHARED_CONTEXT;
        ssv = (volatile SV *)av_pop((AV*)sobj);
        CALLER_CONTEXT;
        ST(0) = sv_newmortal();
...
STORESIZE(SV *obj,IV count)
    CODE:
        dTHXc;
        volatile SV *ssv = SHAREDSV_FROM_OBJ(obj);

        SHARED_EDIT;
        assert(SvTYPE((SV *)ssv) == SVt_PVAV);
        if (!PL_dirty) {
...
_id(SV *myref)
    ...
    CODE:
        ...
        if (! ssv)
            XSRETURN_UNDEF;
        ST(0) = sv_2mortal(newSVuv(PTR2UV((SV *)ssv)));
        /* XSRETURN(1); - implied */
...
_refcnt(SV *myref)
    ...
    CODE:
        ...
        ST(0) = sv_2mortal(newSViv(SvREFCNT((SV *)ssv)));
        /* XSRETURN(1); - implied */
    ...
    CODE:
        ...
        if (ssv) {
            ...
            SHARED_CONTEXT;
            {
                ...
                (void)sv_bless((SV *)ssv, (HV*)fake_stash);
            }
            CALLER_CONTEXT;
            LEAVE_LOCK;
        }
